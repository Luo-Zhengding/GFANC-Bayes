...
# Bayesian formula
def Construct_filter(sub_filters, soft_labels_pre, soft_labels_now):
    ...
    novel_filter = np.matmul(soft_labels, sub_filters)
...
class Control_filter_Index_predictor():
    ...
    def predic_ID(self, noise, soft_labels_pre): # predict the noise index
        ...
        construt_filter, soft_labels = Construct_filter(self.sub_filters_T.detach().numpy(), soft_labels_pre, soft_labels_now=prediction.detach().cpu().numpy())
    ...
    def predic_ID_vector(self, primary_noise):
        ...
        soft_labels_vector = np.full((Time_len+1, 15), 0.5)
        
        #save initial control filter
        Initial = np.matmul(soft_labels_vector[0], self.sub_filters_T.detach().numpy())
        savemat('models/Initial_Control_Filter.mat', {'Initial': Initial})
        
        for ii in range(Time_len):
            construt_filter, soft_labels = self.predic_ID(primary_noise_vectors[ii], soft_labels_vector[ii])
...
def Control_filter_selection_pre_now(fs, MODEL_PTH, path_mat, Primary_noise):
